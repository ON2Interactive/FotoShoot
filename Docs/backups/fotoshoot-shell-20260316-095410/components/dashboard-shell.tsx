...
export default function DashboardShell() {
  ...
  const isProcessingModalOpen = activeImage?.status === "processing";
  ...
  async function handleStudioShoot() {
    if (!activeImage || activeImage.status === "processing") {
      return;
    }
  ...
  function handleDownload() {
    ...
    link.download = `fotoshoot-${activeImage.name}`;
    link.click();
  }
      ...
      <header className="workspace-header">
        <div className="workspace-header-brand">
          ...
          <div className="workspace-header-copy">
            <span>FotoShoot</span>
            <small>Nano Banana workspace</small>
          </div>
        </div>
