...
function buildPrompt(mode: EditMode, userPrompt?: string) {
  ...
  if (mode === "studio_shoot") {
    return [
      ...
      "Treat product fidelity as a hard requirement.",
      "Do not alter the product itself.",
      "Do not change the product shape, silhouette, proportions, branding, label text, material finish, construction details, or core color identity.",
      "Preserve logos, packaging marks, stitching, hardware, edges, and any sellable details exactly as provided.",
      "If the image contains a product, only restyle the environment around it: lighting, shadows, background, surface, and composition.",
      ...
      userPrompt ? `Studio direction: ${userPrompt}` : null,
